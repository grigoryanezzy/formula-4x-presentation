...
(() => {
  ...
  const stylesheet = `
    ...
    /* ── Mobile (≤820px): stack every slide full-width, vertical page scroll.
       Each 1920×1080 slide is scaled to viewport width via transform; the
       leftover layout box is collapsed with a negative margin and clipped
       horizontally. Toggled by the .ds-mobile class set in _fit(). ────────── */
    @media (max-width: 820px) {
      ...
      :host(.ds-mobile) ::slotted(*) {
        position: relative !important;
        inset: auto !important;
        display: block !important;
        width: var(--deck-design-w) !important;
        height: var(--deck-design-h) !important;
        transform: scale(var(--ds-mobile-scale, 0.2)) !important;
        transform-origin: top left !important;
        margin: 0 0 calc((var(--ds-mobile-scale, 0.2) - 1) * var(--deck-design-h)) 0 !important;
        opacity: 1 !important;
        visibility: visible !important;
        pointer-events: auto !important;
      }
      :host(.ds-mobile) ::slotted([data-deck-skip]) { display: none !important; }
      :host(.ds-mobile) .rail,
      :host(.ds-mobile) .rail-resize,
      :host(.ds-mobile) .overlay,
      :host(.ds-mobile) .tapzones,
      :host(.ds-mobile) .ctxmenu,
      :host(.ds-mobile) .confirm-backdrop { display: none !important; }
    }
  `;
  ...
  class DeckStage extends HTMLElement {
    ...
    _fit() {
      ...
      // MOBILE: vertical scroll-feed — all slides full-width, stacked; page scrolls.
      if (!this._presenting && !this._previewMode && window.matchMedia('(max-width: 820px)').matches) {
        this.classList.add('ds-mobile');
        this.style.setProperty('--ds-mobile-scale', String(window.innerWidth / this.designWidth));
        this._canvas.style.transform = 'none';
        if (stage) stage.style.left = '0';
        if (this._overlay) this._overlay.style.marginLeft = '0';
        if (this._tapzones) this._tapzones.style.left = '0';
      ...
      this.classList.remove('ds-mobile');
      this.style.removeProperty('--ds-mobile-scale');
      const rw = this._railWidth();
      const cameraOffset = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--ds-left-offset')) || 0;
      const stageLeft = rw + cameraOffset;
      if (stage) stage.style.left = stageLeft + 'px';
      // Overlay is centred on the viewport via left:50% + translate(-50%);
      // marginLeft shifts the centre into the [stageLeft, innerWidth] stage region.
      if (this._overlay) this._overlay.style.marginLeft = (stageLeft / 2) + 'px';
      if (this._tapzones) this._tapzones.style.left = stageLeft + 'px';
      const hostRect = this.getBoundingClientRect();
      // If the host element has been styled smaller than the window (e.g. video-mode
      // wraps the deck in the right 2/3), use the host's own width/height for scaling
      // instead of the full window.
      const useHost = hostRect.width > 0 && hostRect.width < window.innerWidth - 1;
      const vw = useHost ? hostRect.width - stageLeft : (window.innerWidth - stageLeft);
      const vh = useHost ? hostRect.height : window.innerHeight;
